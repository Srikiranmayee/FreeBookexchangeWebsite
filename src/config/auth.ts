...
export const validateAuthConfig = () => {
  ...
  if (!authConfig.google.clientId || authConfig.google.clientId.includes('your-google-client-id')) {
    errors.push('Google Client ID is not configured. Please set REACT_APP_GOOGLE_CLIENT_ID environment variable.');
  }
  
  if (!authConfig.apple.clientId || authConfig.apple.clientId.includes('your.apple.service.id')) {
    errors.push('Apple Client ID is not configured. Please set REACT_APP_APPLE_CLIENT_ID environment variable.');
  }
